    ...
    <header className="sticky top-0 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800 transition-colors">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        >
          ...
          <div>
            <div className="flex items-center gap-1.5">
              ...
              <span className="hidden sm:inline-block px-2 py-0.5 text-[11px] font-medium bg-rose-100 text-rose-700 dark:bg-rose-950/60 dark:text-rose-300 rounded-full">
                生活足跡・日誌
              </span>
            </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          {/* Icon 1: 關於 Icon (Link to https://sites.google.com/view/joyiverse) */}
          <a
            href="https://sites.google.com/view/joyiverse"
            target="_blank"
            rel="noopener noreferrer"
            className="p-2.5 rounded-xl text-slate-600 dark:text-slate-300 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all flex items-center gap-1.5 focus:outline-none"
          ...
          <div className="relative">
            {user ? (
              <button
                ...
                className="flex items-center gap-2 p-1 pl-2 pr-1.5 rounded-full border border-slate-200 dark:border-slate-700 hover:border-rose-300 dark:hover:border-rose-700 bg-slate-50 dark:bg-slate-800/80 transition-all focus:outline-none cursor-pointer"
                id="header-user-avatar-btn"
              >
                <span className="text-xs font-medium text-slate-700 dark:text-slate-200 max-w-[90px] truncate hidden sm:inline">
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 hover:bg-rose-500 hover:text-white dark:hover:bg-rose-600 transition-all focus:outline-none cursor-pointer"
            ...
            {isProfileOpen && user && (
              <>
                ...
                />
                <div className="absolute right-0 mt-2 w-72 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200/80 dark:border-slate-800 p-4 z-50 animate-in fade-in zoom-in-95 duration-150">
                  <div className="flex items-center gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
                    {user.photoURL ? (
                      <img
                        ...
                        alt=""
                        referrerPolicy="no-referrer"
                        className="w-11 h-11 rounded-full object-cover ring-2 ring-rose-500/20"
                      />
                    ) : (
                  ...
                  <div className="py-2.5 space-y-1.5 text-xs text-slate-600 dark:text-slate-300">
                    <div className="flex items-center justify-between p-2 rounded-xl bg-slate-50 dark:bg-slate-800/60">
                      ...
                      <span className="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300 text-[10px] font-medium">
                        Google Gmail 已驗證
                      </span>
                    </div>
                    ...
                    <div className="p-2 rounded-xl bg-slate-50 dark:bg-slate-800/60">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[11px] text-slate-500">預設公開</span>
                      </div>
