...
export const PersonalLogsView: React.FC<PersonalLogsViewProps> = ({ showToast }) => {
  ...
  const [timeFilter, setTimeFilter] = useState<TimeRangeFilter>('month');
  ...
  // Filter logs according to TimeRange (Day, Week, Month, Year)
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            按日、週、月、年篩選 6 大類別總量，支援編輯備註與刪除管理
          </p>
        </div>
      ...
      {/* Time Dimension Switcher Tabs (日 / 週 / 月 / 年) */}
      <div className="p-1.5 rounded-2xl bg-slate-100 dark:bg-slate-800/80 flex items-center gap-1 max-w-md">
        {(
          [
            { id: 'day', label: '今日 (Day)' },
            { id: 'week', label: '近 7 天 (Week)' },
            { id: 'month', label: '近 30 天 (Month)' },
            { id: 'year', label: '近 1 年 (Year)' },
