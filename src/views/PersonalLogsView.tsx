...
export const PersonalLogsView: React.FC<PersonalLogsViewProps> = ({ showToast }) => {
  ...
  // Track active target email (from login or localStorage)
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            按全部、日、週、月、年篩選 6 大類別總量，支援編輯備註與刪除管理
          </p>
        </div>
        ...
        {user ? (
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-emerald-100 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 flex items-center justify-center font-bold text-sm">
                {user.photoURL ? (
            ...
            <button
              ...
              className="px-3.5 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-300 text-xs font-medium flex items-center gap-1.5 transition-colors cursor-pointer self-start sm:self-auto"
            >
          ...
          <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                {activeEmail && (
                  <span className="px-2 py-0.5 rounded-full bg-violet-100 dark:bg-violet-950/60 text-violet-700 dark:text-violet-300 text-[10px] font-bold">
                    已連結：{activeEmail}
                  </span>
                )}
              </div>
              ...
              >
                ...
                <span>點此直接登入 Google</span>
              </button>
            </div>

            <form onSubmit={handleSyncEmail} className="flex items-center gap-2">
              <div className="relative flex-1 min-w-0">
                <input
                  type="email"
                  placeholder="輸入您的 Gmail 載入全站 6 大生活日誌數據..."
                  value={inputEmail}
                  onChange={(e) => setInputEmail(e.target.value)}
                  className="w-full pl-3.5 pr-3 py-2 text-xs rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 outline-none"
                />
              </div>
              <button
                type="submit"
                disabled={loading || !inputEmail.trim()}
                className="px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white text-xs font-bold rounded-xl transition-colors flex items-center gap-1.5 cursor-pointer disabled:opacity-50 flex-shrink-0"
              >
                <Search className="w-3.5 h-3.5" />
                <span>{loading ? '同步中...' : '同步載入'}</span>
              </button>
            </form>
          </div>
        )}
